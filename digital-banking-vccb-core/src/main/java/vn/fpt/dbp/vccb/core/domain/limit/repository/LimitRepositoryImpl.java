...
public class LimitRepositoryImpl implements DBPRepository<Limit>{
	@PersistenceContext
	...
	*/
	public PagedResource<Limit> searchLastedVersion(Limit limit, Pageable pageable){
		...
		if(StringUtils.isNotEmpty(limit.getCode())){
			parCode = limit.getCode();
		}
		if(StringUtils.isNotEmpty(limit.getName())){
			parName = limit.getName();
		}
		if(StringUtils.isNotEmpty(limit.getWorkflowStatus())){
			parWorkFlowStatus = StringUtils.upperCase(limit.getWorkflowStatus());
		}
		if(limit.getCreatedBy() != null && limit.getCreatedBy().getId() != null){
			parCreatedBy = limit.getCreatedBy().getId().toString();
		}
		...
		sql.append(" FROM LIMIT LM JOIN WORKFLOW_MODEL WF ON LM.ID = WF.ID ");
		...
		sqlCount.append(" FROM LIMIT LM JOIN WORKFLOW_MODEL WF ON LM.ID = WF.ID ");
		...
		Query query = em.createNativeQuery(sql.toString(),Limit.class);
		...
		List<Limit> limits = query.getResultList();
		...
		PagedResource<Limit> result = new PagedResource<Limit>(limits,pageable.getPageNumber(),pageable.getPageSize(),totalPages,totalRows);
