...
public class ExchangeRepositoryImpl implements DBPRepository<Exchange> {
    @PersistenceContext
    private EntityManager em;

    public PagedResource<Exchange> searchLastedVersion(Exchange exchange, Pageable pageable) {
        ...
        sqlBody.append(" (SELECT WF.*,  EX.AREA, EX.BRANCH, EX.CURRENCY_FROM, EX.CURRENCY_TO, EX.EXCHANGE_UPLOAD_FILE, " );
        sqlBody.append("  MAX(CREATED_DATE) OVER(PARTITION BY ORIGINAL_ID, AREA, BRANCH, CURRENCY_FROM, CURRENCY_TO) ");
        sqlBody.append("  MAX_DATE ");
            //Join
        sqlBody.append("  FROM EXCHANGE EX JOIN WORKFLOW_MODEL WF ON EX.ID = WF.ID " );
        ...
        Query query = em.createNativeQuery(sqlQuery.toString(), Exchange.class);
        ...
        PagedResource<Exchange> result = new PagedResource<Exchange>(exchanges,pageable.getPageNumber(),pageable.getPageSize(),totalPages, totalRows);
        return result;
    }
}
